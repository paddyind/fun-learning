...
- **Auth:** NextAuth.js v4 with the Keycloak OIDC provider (`lib/auth.ts`). Route protection via `middleware.ts` (matcher `/dashboard/:path*`). **A local Keycloak runs in Docker by default** (`docker-compose.yml`'s `keycloak` service, realm imported from `keycloak/realm-export.json`, test user `parent1`/`parent12345`) — real OIDC login works out of the box for local testing, no external Keycloak needed. See `docs/architecture.md` → "Local Keycloak" for the dual-hostname fix (`extra_hosts: host-gateway`) that makes this work.
...
1. `npm install`
2. Fill in `.env.local` — Firebase (client config + service account, for real Auth) and Gemini are required for the app to actually *work*. Keycloak needs nothing from you: the `KEYCLOAK_*` values already in `.env.local` point at the local Docker Keycloak (test user `parent1`/`parent12345`), and the demo account (`/help`) works too as a lighter-weight alternative.
3. Firestore/Storage are emulated by default (`NEXT_PUBLIC_USE_FIREBASE_EMULATORS=true`) — no rules deployment needed for local testing, `docker compose up` starts the emulator (and local Keycloak) too. Only deploy `firestore.rules`/`firestore.indexes.json`/`storage.rules` for real (`npx firebase-tools deploy --only firestore:rules,firestore:indexes,storage`) once you switch that flag to `false`.
