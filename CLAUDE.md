...
- **Auth:** NextAuth.js v4 with the Keycloak OIDC provider (`lib/auth.ts`). Route protection via `middleware.ts` (matcher `/dashboard/:path*`). **Keycloak itself is not part of this repo** — it's owned by the sibling `identity-platform` project (a shared multi-app Keycloak deployment), started separately (`cd ../identity-platform && docker compose up -d`). fun-learning's realm/client/test-user are already defined there (`realms/fun-learning-realm.json`, test user `parent1@example.com`/`parent12345`). The demo login (below) still works with zero setup if you don't want to start identity-platform at all. See `docs/architecture.md` → "Keycloak lives outside this repo" for the endpoint-splitting fix (`lib/auth.ts`: public `issuer`/`authorization` vs internal `token`/`userinfo`/`jwks_endpoint`) that makes browser + server reach it correctly.
...
1. `npm install`
2. Fill in `.env.local` — Firebase (client config + service account, for real Auth) and Gemini are required for the app to actually *work*. For Keycloak, either start the sibling `identity-platform` repo (`cd ../identity-platform && docker compose up -d`) — its `KEYCLOAK_*` values already match `.env.local` (test user `parent1@example.com`/`parent12345`) — or skip it and use the demo account (`/help`) instead.
3. Firestore/Storage are emulated by default (`NEXT_PUBLIC_USE_FIREBASE_EMULATORS=true`) — no rules deployment needed for local testing, `docker compose up` starts the emulator too (not Keycloak — that's identity-platform's job). Only deploy `firestore.rules`/`firestore.indexes.json`/`storage.rules` for real (`npx firebase-tools deploy --only firestore:rules,firestore:indexes,storage`) once you switch that flag to `false`.
